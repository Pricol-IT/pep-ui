...
function initializeCalendar() {
  ...
  function animateCalendarChange(direction) {
    ...
    calendarGrid.style.transform = `translateX(${
...
function initializeLazyLoading() {
  ...
  const cards = document.querySelectorAll(
    ".overview-card, .quick-links-card, .team-card, .calendar-card"
  );
  cards.forEach((card) => {
    ...
    try {
      const check = () =>
        document.fonts && document.fonts.check
          ? document.fonts.check('1em "tabler-icons"')
          : true;
      const swapToFA = () => {
        const map = {
          "ti-sparkles": "fa-wand-magic-sparkles",
...
(function setupAppsToolbar() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const search = document.querySelector(
      ".applications-section .search-container input"
    );

    const getCards = () =>
      Array.from(grid.querySelectorAll(".app-card")).filter(
        (el) => !el.classList.contains("add-app")
      );
...
(function ensureWeatherInit() {
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function run(lat, lon) {
      try {
        const data = await (window.__fetchOpenMeteo
          ? window.__fetchOpenMeteo(lat, lon)
          : null);
        if (data) {
          ...
          const key =
            typeof codeToIconKey === "function"
              ? codeToIconKey(data.code)
              : "cloudy";
          const map = {
            clear: "ti-sun",
            cloudy: "ti-cloud",
...
function copyEmpCode() {
  copyToClipboard("EMP001234");
}
